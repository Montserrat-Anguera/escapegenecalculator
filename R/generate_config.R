...
# required defaults
mat_reads_dir = file.path(in_dir, "read_counts/mat_reads")
pat_reads_dir = file.path(in_dir, "read_counts/pat_reads")
rpkms_dir = file.path(in_dir, "read_counts/rpkms")
...
df = as.data.frame(do.call(cbind,
    list(mat_reads=mat_reads_files,
         pat_reads=pat_reads_files,
         rpkms=rpkms_files))
)
